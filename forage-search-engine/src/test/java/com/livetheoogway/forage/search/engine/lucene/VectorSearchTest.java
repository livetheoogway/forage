...
class VectorSearchTest {
    ...
    @Test
    void testHybridSearchWithFilterMatchingNoDocuments() throws Exception {
        try (ForageLuceneSearchEngine<VectorDocument> engine = buildVectorEngine(
                vectorDoc("apple-doc", "apple", new float[]{1.0f, 0.0f, 0.0f, 0.0f}),
                vectorDoc("banana-doc", "banana", new float[]{0.9f, 0.1f, 0.0f, 0.0f}))) {

            float[] queryVector = new float[]{1.0f, 0.0f, 0.0f, 0.0f};

            // Filter that matches none of the indexed documents
            ForageQuery query = QueryBuilder.knnQuery("embedding", queryVector, 5)
                    .filter(QueryBuilder.matchQuery("content", "mango").build())
                    .buildForageQuery();
